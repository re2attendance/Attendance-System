...
- **Declared days** (ADR-012). `declare_calendar_event()` is the only door. Scope is the rule: `class_section_id` null = institution-wide = **admin only**; set = that section = its rep/instructor/admin. A rep is a student — "reps can declare a holiday" must never mean a student can close the university. **An emergency is today only** (institution's timezone), single day. **Nothing is backdated** — a retroactive declaration is indistinguishable from erasing that day's absences. Declaring cancels the day's sessions and voids every record on them *including approved ones*; the approval itself is kept, only the status changes. The reason lives on the session (`cancelled_by_event_id`), never as a new status.
- **`submitted_at` is immutable after insert.** It is stamped server-side on INSERT and never moves again. An UPDATE cannot change it — not by a rep, not by an approval, not by anyone with a JWT. It is the one input `deriveStatus` anchors on, so anything that can move it can choose a student's status.
